...
4. Centroids can be linked using the Fasta-UC linker. The linked centroids can then have their nucleotide sequences acquired by the bash wrapper that works with get.contigs to output a fasta file containing the nt sequence and metadata (./Wrapper.sh "target file"). This will work for any .tsv file you feed it so long as it has an SRA accession.
